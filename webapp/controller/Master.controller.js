/*global history */
sap.ui.define([
	"de/wegelin/FioriCRUD/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"sap/m/GroupHeaderListItem",
	"sap/ui/Device",
	"de/wegelin/FioriCRUD/model/formatter",
	"sap/m/MessageBox",
	"de/wegelin/FioriCRUD/model/grouper",
	...
	return BaseController.extend("de.wegelin.FioriCRUD.controller.Master", {
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("so_id", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function () {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("de.wegelin.FioriCRUD.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function (oEvent) {
			...
			aFilterItems.forEach(function (oItem) {
				switch (oItem.getKey()) {
				case "Filter1":
					aFilters.push(new Filter("net_amount", FilterOperator.LE, 100));
					break;
				case "Filter2":
					aFilters.push(new Filter("net_amount", FilterOperator.GT, 100));
					break;
				default:
					break;
				}
				...
				function (mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							node_key: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("node_key"))
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("object", {
				node_key: encodeURIComponent(oItem.getBindingContext().getProperty("node_key"))
			}, bReplace);
		},
